...
namespace OR{
enum class RMP_t : uint32_t {
    RMP_0 = 0, // field has value 0x0
RMP_1 = 1, // field has value 0x1
RMP_2 = 2, // field has value 0x2
RMP_3 = 3, // field has value 0x3
...
class OR : public ReadableWritable<address> {
   public:
    ...
    RMP_t RMP(){
    uint32_t tmp = this->bits_masked(0x3) >> 0;
    return reinterpret_cast<RMP_t>(tmp);
}
void RMP(RMP_t v){
   uint32_t tmp = v << 0;
   this->bits_masked(v, 0x3);
