...
namespace OR{
enum class ITR1_RMP_t : uint32_t {
    ITR1_RMP_0 = 0, // field has value 0x0
ITR1_RMP_1 = 1, // field has value 0x1
ITR1_RMP_2 = 2, // field has value 0x2
ITR1_RMP_3 = 3, // field has value 0x3

};
template <uint32_t address>
class OR : public ReadableWritable<address> {
   public:
    OR() : ReadableWritable<address>() {}
    ITR1_RMP_t ITR1_RMP(){
    uint32_t tmp = this->bits_masked(0xc00) >> 10;
    return reinterpret_cast<ITR1_RMP_t>(tmp);
}
void ITR1_RMP(ITR1_RMP_t v){
   uint32_t tmp = v << 10;
   this->bits_masked(v, 0xc00);
}
};
